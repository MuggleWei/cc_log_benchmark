...
以上 4 个日志库均为异步日志库, 设计上也都是多缓冲区队列写, 消费者负责轮询的模式, 整体思路相似, 代码细节各有各的趣味.  
...
### 额外的一点考虑
haclog 使用纯 C 开发, 而 fmtlog, Nanolog 和 quill 采用 C++ 开发; 在日志前端序列化方面, C 需要在运行时遍历 va_list, 生成的汇编代码是通过循环来逐个参数序列化, 无论是通过 switch 判断预计算好的类型，或是函数指针直接调用, 都会产生一点额外的预测开销; 而 C++ 通过变参模板来进行序列化, 汇编代码可实现为平展的形式, 以空间换时间; 所以就日志前端序列化速度来说, 理论上 C++ 实现的日志库上限会更高一些  
