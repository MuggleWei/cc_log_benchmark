...
## 基准测试结果
在 [gbenchmark](./report/benchmark_20250214/gbenchmark) 目录中, 可以找到我本地机器的基准测试报告的详细信息, 图表化表示如下:  
...
而由于 [额外说明](#额外说明) 中提到的情况, 导致 fmtlog 只可选择方案 2, quill 仅讨论非扩展的模式  

### fmtlog
优点
* 在两种场景下都表现出良好的速度, 其中在 **场景1** 中速度与 quill 并列最快速度
...
优点
* 在两种场景下都表现出良好的速度
* 很高的吞吐量, 在 **场景1** 虽然效率有所下降, 但是与其他同步日志相比依旧有巨大优势
* 缓冲区满时采用了阻塞模式, 不会丢日志
...
### quill
优点
* 在两种场景下都表现出良好的速度, 其中在 **场景1** 中速度与 fmtlog 并列最快速度
* 在两种场景下, 速度表现的稳定性足够好, 波动较小
* 采用了 format 格式化风格
* 支持缓冲区满时采用了阻塞模式, 虽然效率有所下降，但可以不丢日志
...
## 综上所述
* fmtlog 和 quill 采用 format 格式化风格, 书写方便, 且两者速度稳定性较强
  * 在 **场景1** 表现出色. 但是如上一小节所分析的, 此场景的速度部分是通过日志丢失换取的, 这在使用场景上需要特别注意； 而开启阻塞模式时, **场景1** 在我的机器上没法完成
  * 两者也都支持缓冲区满时采用阻塞模式, 在 **场景2** 中, fmtlog 开阻塞模式时, 测试无法完成, 而 quill 开阻塞模式时, 效率有所下降, 但还是十分优秀  
* haclog 在 **场景2** 中的表现良好, 但是在 **场景1** 当缓冲区被压满的情况下, 可以保证不丢日志, 但是效率大幅度的降低  
* Nanolog 在两种场景下都表现出很高的效率以及超高的吞吐量, 面临压力的情况下, 保证不丢日志也不用拿太多的效率去换. 不过其超高的吞吐量, 是拿日志非实时可读换取的, 这也导致诸如 `tail` 一类的工具无法使用
